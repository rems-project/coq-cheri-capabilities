(*Generated by Sail from capfns, with imported SailStdpp in place of Sail.*)
Require Import SailStdpp.Base.
Require Import SailStdpp.Real.
(*Require Import capfns_types.*)
From CheriCaps Require Import CapFnsTypes_Sail_gen.
...
: M ((mword (63 - 0 + 1 + 1) * mword (63 - 0 + 1 + 1) * bool)) :=
   let '(exp) := CapGetExponent c in
   (if Z.eqb exp CAP_MAX_ENCODEABLE_EXPONENT then returnM (CAP_BOUND_MIN, CAP_BOUND_MAX, true)
      ...
      assert_exp' (Z.ltb (Z.sub (Z.add (__id exp) 16) 1) 66) "v8_base.sail 1391:41 - 1391:42" >>= fun _ =>
      let base : bits 66 :=
...
Definition CapBoundsEqual (a : mword 129) (b : mword 129) : M (bool) :=
   ...
   let '((tup__0, tup__1, tup__2)) :=
     w__0
      : (mword (63 - 0 + 1 + 1) * mword (63 - 0 + 1 + 1) * bool) in
...
Definition CapSetValue (c__arg : mword 129) (v : mword (63 - 0 + 1)) : M (mword 129) :=
   ...
   let c : mword 129 := if negb w__0 then CapWithTagClear c else c in
...
: M (bool) :=
   ...
   let '(exp) := CapGetExponent c in
   returnM (if Z.geb exp (Z.sub CAP_MAX_EXPONENT 2) then true
            else
...
Definition CapAdd (c : mword 129) (increment_name : mword (63 - 0 + 1)) : M (mword 129) :=
   ...
   let newc : mword 129 :=
     if negb w__0 then update_vec_dec newc CAP_TAG_BIT (Bit (('b"0")  : mword 1))
     else newc in
   let newc : mword 129 :=
...
: M (mword 129) :=
   ...
   (if ie return M ((mword (79 - 64 + 1) * mword (79 - 64 + 1) * Z * bool * bool)) then
      let '(exp) := exp in
      assert_exp' (andb (Z.leb 0 (Z.add (__id exp) 3)) (Z.ltb (Z.sub (Z.add (__id exp) 16) 1) 66)) "v8_base.sail 45081:69 - 45081:70" >>= fun _ =>
      let B_ie : bits (CAP_MW - 3) :=
        autocast (subrange_vec_dec req_base (Z.sub (Z.add exp CAP_MW) 1) (Z.add exp 3)) in
      let '(exp) := exp in
      assert_exp' (andb (Z.leb 0 (Z.add (__id exp) 3)) (Z.ltb (Z.sub (Z.add (__id exp) 16) 1) 66)) "v8_base.sail 45086:69 - 45086:70" >>= fun _ =>
      let T_ie : bits (CAP_MW - 3) :=
        autocast (subrange_vec_dec req_top (Z.sub (Z.add exp CAP_MW) 1) (Z.add exp 3)) in
      let '(exp) := exp in
      assert_exp' (Z.geb (Z.add (__id exp) 3) 0) "v8_base.sail 45091:46 - 45091:47" >>= fun _ =>
      assert_exp' (Z.geb 66 (Z.add (__id exp) 3)) "v8_base.sail 45092:47 - 45092:48" >>= fun _ =>
      ...
      let T_ie : mword (79 - 64 + 1 - 3) := if lostTop then add_vec_int T_ie 1 else T_ie in
      ...
      (if eq_vec (vec_of_bits [access_vec_dec L_ie (Z.sub CAP_MW 4)]  : mword 1)
         ...
         assert_exp' (Z.leb 0 (Z.add (__id exp) 4)) "v8_base.sail 45108:50 - 45108:51" >>= fun _ =>
         let B_ie : bits (CAP_MW - 3) :=
...
Definition CapGetRepresentableMask (len : mword (63 - 0 + 1)) : M (mword (63 - 0 + 1)) :=
   ...
   let '(exp1) := exp1 in
   assert_exp' (Z.geb (Z.sub 64 (__id exp1)) 0) "v8_base.sail 45163:38 - 45163:39" >>= fun _ =>
   assert_exp' (Z.geb (__id exp1) 0) "v8_base.sail 45164:33 - 45164:34" >>= fun _ =>
   returnM (autocast (concat_vec (Ones (Z.sub CAP_VALUE_NUM_BITS exp1)) (Zeros exp1))).
